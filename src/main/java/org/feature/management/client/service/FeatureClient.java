    ...
    public Mono<Feature> fetchFeature(String featureName) {
        Mono<Feature> request = webClient.get()
                ...
                .onStatus(status -> status.is4xxClientError() && !status.equals(HttpStatus.NOT_FOUND),
                        res -> Mono.error(new FeatureManagementException("Client error fetching feature details", null)))
                .onStatus(status -> status.is5xxServerError(),
                        res -> Mono.error(new FeatureManagementException("Server error fetching feature details", null)))
